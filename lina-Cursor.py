...
def build_kingdee_voucher(draft_file_obj, date_str):
    """
    将上传的 Cursor 拆分底稿自动转换为金蝶上传凭证模板内存文件
    """
    # 1. 解析日期参数
    ...
    explanation = f"计提{year}年{month}月Cursor消耗-主营业务成本_软件服务费"
    vendor_code = "VEN02189"  # Cursor 供应商编码
    ...
    for item in rows_data:
        ...
        row = [
            ...
            np.nan,                                 # 49
            vendor_code,                            # 50: FDetailID#FFlex4 (分录)供应商#编码 (VEN02189)
    ...
    credit_row = [
        ...
        vendor_code,                                # 50: FDetailID#FFlex4 供应商编码 (VEN02189)
    ...
    # 6. 将生成的 Excel 写入内存数据流中，并正确命名 Sheet 页签为 '凭证#单据头(FBillHead)'
...
# --- Streamlit 网页前端界面 ---
st.title("📊 Cursor 金蝶入账凭证生成工具")

uploaded_file = st.file_uploader("请选择或拖入当月【Cursor拆分底稿】Excel 文件", type=["xlsx", "xls"])
...
if uploaded_file is not None:
    if st.button("🚀 开始自动转换"):
        try:
            ...
            file_name = f"Cursor_金蝶上传凭证_{voucher_date_input.strftime('%Y%m')}.xlsx"
            st.download_button(
                label="📥 点击下载金蝶凭证上传文件",
                data=excel_data,
                file_name=file_name,
